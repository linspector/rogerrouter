...
suggestion_entry_init (SuggestionEntry *self)
{
  ...
  if (!g_object_get_data (G_OBJECT (gdk_display_get_default ()), "suggestion-style"))
    {
      ...
      gtk_css_provider_load_from_resource (provider, "/org/tabos/roger/suggestion-entry.css");
  ...
  gtk_text_set_placeholder_text (GTK_TEXT (self->entry), _("Enter name or number"));
